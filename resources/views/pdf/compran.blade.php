...
<body>
  <div class="Table" style="padding-top: 0px">
    ...
    <div class="Row">
      ...
      text-align: center; background-color: #d2caca"><b>ORDEN DE COMPRA</b> </th>
          ...
          <tr>
            <td>
              @if($compra[0]->fecha_orden > '2023-09-30')
              {{ $compra[0]->nomnree }}
              @elseif($compra[0]->fecha_orden > '2020-06-15')
    ...
    <div class="Row">
      <div class="Cell">
        <p class="pd"><b> PROVEEDOR:</b>&nbsp;{{$compra[0]->razonsocialp }} <br>
          ...
          <br>
          <b>ID:</b>&nbsp;{{str_pad($compra[0]->proveedor_id , 5, "0", STR_PAD_LEFT)}}
        </p>
  ...
  <table width="100%" class="tab">
    <tr>
      <td class="bold" width="5%">Partida</td>
      <td class="bold" width="10%">Cantidad</td>
      <td class="bold" width="10%">Unidad</td>
      <td class="bold" width="55%">Descripción</td>
      <td class="bold" width="5%">Documentos Requeridos</td>
      <td class="bold" width="10%">Precio Unitario</td>
      <td class="bold" width="10%">Total</td>
    </tr>
    ...
    <tr>
      ...
      <td>{{$comp->unidadA}}</td>
      <td>
        {{$comp->descripcionA . ($comp->comentario==null?"":(" - ".$comp->comentario))}}
      </td>
      <td>
        {{isset($comp->documentos)?$comp->documentos:"-"}}
      </td>
  ...
  <table class="tab" width="100%">
    ...
    @foreach($documentos_compra as $doc)
    <tr>
      @foreach($doc as $d)
      <td width="8%">{{$d->nombre_corto}}</td>
      <td width="20%" style="text-align: left;">{{$d->nombre}}</td>
      @endforeach
    </tr>
    @endforeach
  </table>
  ...
  <div style="page-break-inside: avoid;">
    <p style="font-size: 16px;
      font-family: Arial, Helvetica, sans-serif;
      text-align: center;">
      <b>Este pedido se ha creado electrónicamente y ha sido aprobado por las siguientes personas:</b>
    </p>
    <table width="100%">
      <tr>
