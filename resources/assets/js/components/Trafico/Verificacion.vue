        ...
        <div class="card-body">
            ...
            <template>
                ...
                <div class="form-row">
                    <div class="form-group col-md-4">
                        <label>Costo</label>
                        <input type="number" class="form-control" v-validate="'required|decimal:2'" data-vv-name="Costo" v-model="verificar.costo">
                        <span class="text-danger">{{errors.first('Costo')}}</span>
                    </div>
                    <div class="form-group col-md-4">
                        <label>Multa</label>
                        <input type="number" class="form-control" v-model="verificar.costo_multa">
                    </div>
                    <div class="form-group col-md-4">
            ...
            <!-- Inicio de tabla de  -->
            <vue-element-loading :active="isDocumentoLoading" />
            <div>
                <v-client-table :data="dataverificar" :options="optionsverificar" :columns="columnsverificar" ref="tablePoliza">
                    ...
                    <template slot="comprobante" slot-scope="props">
                        <template v-if="props.row.comprobante==null">
                            <button class="btn btn-dark">
                                N/D
                            </button>
                        </template>
                        <template v-else>
                            <button class="btn btn-dark" @click="descargar(props.row.comprobante)">
...
export default
{
    ...
    methods:
    {
        ...
        {
            this.isLoading = true;
            axios.get('vehiculos/verificaciones/' + this.id).then(response =>
            {
        ...
        guardarMto(nuevo)
        {
            this.$validator.validate().then(result =>
            {
                if (result)
                {
                    ...
                    axios.post('vehiculos/verificaciones', formData).then(response =>
                    {
                        me.isLoading = false;
