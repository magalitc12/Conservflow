    ...
    <div class="">
        <div class="card" v-show="detalle == 1">
            <div class="card-header">
                <i class="fa fa-align-justify"></i> CFDI 4.0 - {{usuario_texto}}
                <div class="dropdown float-sm-right">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdown">
                        <div v-for="elem in list_emisores" :key="elem.key">
                            <button class="dropdown-item" @click="emisor_id=elem.id;verfacturas(elem.id)">{{elem.nombre}}</button>
                        </div>
                    </div>
                </div>
                ...
                <button type="button" class="btn btn-dark float-sm-right" @click="abrirModal()">
                                ...
                                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                    <template v-if="props.row.timbrado == 0">
                                        <button type="button" @click="abrirModal(false,props.row)" class="dropdown-item">
                                            <i class="fas fa-edit"></i> Actualizar Factura
                                        </button>
                                        <button type="button" v-show="props.row.descripcion_tipofactura === 'Pago'" @click="agregarP(props.row)" class="dropdown-item">
                                            <i class="fas fa-list-ol"></i> Agregar Pagos
                                        </button>
                                        <button type="button" v-show="props.row.descripcion_tipofactura === 'Ingreso' 
                                        || props.row.descripcion_tipofactura === 'Egreso'" @click="agregarI(props.row)" class="dropdown-item">
                                            <i class="fas fa-list"></i> Agregar Conceptos
                                        </button>
                                        <button type="button" @click="descargarprefactura(props.row)" class="dropdown-item">
                                            <i class="fas fa-download"></i> Descargar prefactura
                                        </button>
                                        <button type="button" @click="TimbrarV4(props.row,false)" class="dropdown-item">
                                            <i class="fas fa-bell"></i> Timbrar 4.0
                                        </button>
                                        <button type="button" @click="TimbrarV4(props.row)" class="dropdown-item">
                                            <i class="fas fa-warning"></i> Timbrar Prueba
                                        </button>
                                    </template>
                                    <!-- Timbrado -->
                                    <template v-if="props.row.timbrado==1">
                                        ...
                                        <button type="button" v-show="props.row.timbrado == 1" @click="DescargarPDF4(props.row)" class="dropdown-item">
                                        ...
                                        <button type="button" v-show="props.row.timbrado == 1" @click="CancelarFactura(props.row,false)" class="dropdown-item">
                                            <i class="fas fa-close"></i> Cancelar factura
                                        </button>
                                    </template>
                                    <template v-if="props.row.timbrado==9">
                                        <button type="button" @click="CancelarFactura(props.row)" class="dropdown-item">
                                            <i class="fas fa-close"></i> Cancelar Prueba
                                        </button>
                                        <button type="button" @click="DescargarXmlPrueba(props.row)" class="dropdown-item">
                                            <i class="fas fa-download"></i> Descargar XML Prueba
                                        </button>
                                        <button type="button" @click="DescargarPDF4(props.row)" class="dropdown-item">
                                            <i class="fas fa-download"></i> Descargar
                                        </button>
                                    </template>
                                </div>
                    ...
                    <template slot="timbrado" slot-scope="props">
                        <template v-if="props.row.timbrado == 0">
                            <button type="button" class="btn btn-outline-info">
                                <i class="fa fa-files-o"></i> Nuevo
                            </button>
                        </template>
                        <template v-if="props.row.timbrado == 1">
                            <button type="button" class="btn btn-outline-success">
                                <i class="fa fa-exclamation-circle"></i> Timbrado
                            </button>
                        </template>
                        <template v-if="props.row.timbrado == 2">
                            <button type="button" class="btn btn-outline-danger">
                                <i class="fa fa-window-close"></i> Cancelado
                            </button>
                        </template>
                        <template v-if="props.row.timbrado == 9">
                            <button type="button" class="btn btn-outline-warning">
                                <i class="fa fa-exclamation-circle"></i> Prueba
                            </button>
                        </template>
                    </template>
                </v-server-table>
                        ...
                        <form>
                            <div class="form-row">
                                <div class="form-group col-md-4">
                                    ...
                                    <select class="form-control" v-model="factura.emisor_id" @click="guardarDG()">
                                        <option v-for="elem in list_emisores" :key="elem.id" :value="elem.id">{{elem.rfc}}</option>
                                    </select>
                                </div>
                                <div class="form-group col-md-6">
                                    <label>Nombre comercial </label>
                                    <input type="text" class="form-control" v-model="DatosGenerales.nombre" placeholder="Nombre comercial" readonly>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-8">
                                    <label>Nombre o razon social </label>
                                    <input type="text" class="form-control" v-model="DatosGenerales.razon_social" placeholder="Nombre o razon social" readonly>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-8">
                                    <label>Regimen fiscal</label>
                                    <input type="text" class="form-control" v-model="DatosGenerales.regimenfiscal" placeholder="Regimen fiscal" readonly>
                                </div>
                                <div class="form-group col-md-4">
                                    <label>Tipo de factura</label>
                                    <select class="form-control" v-model="factura.tipo_factura_id">
                                        <option :key="index" v-for="(item,index) in TipoFactura" :value="item.id">{{item.c_tipofactura}} {{item.descripcion}}</option>
                                    </select>
                                </div>
                            </div>
                            <hr>
                            <div class="form-row" v-show="factura.tipo_factura_id == 1 || factura.tipo_factura_id == 2">
                                <div class="form-group col-md-7">
                                    ...
                                    <select class="form-control" v-model="factura.tipo_relacion">
                                        <option :key="index" v-for="(item,index) in TipoRelacion" :value="item.id">{{item.c_tiporelacion}} {{item.descripcion}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-row" v-show="factura.tipo_factura_id == 1 || factura.tipo_factura_id == 2">
                            ...
                            <div class="form-row" v-show="factura.tipo_factura_id == 1 || factura.tipo_factura_id == 2">
                                <div class="form-group col-md-7">
                                    ...
                                    <v-select :options="optionsvsex" id="relaccionados" name="relaccionados" v-model="factura.factura_id" label="name" data-vv-name="relacionados" v-show="exis"></v-select>
                                </div>
                                <div class="form-group col-md-2">
                                    <label></label>
                                    <br>
                            ...
                            <li :key="index" v-for="(vi, index) in array_relacionados" class="list-group-item">
                                <div class="form-row">
                                    <div class="form-group col-md-5">
                                        <label>{{vi.name}}</label>
                                    </div>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label>Cliente </label>
                                    <v-select v-model="cliente" :options="Clientes" @input="llenarDatosCliente" v-validate="'required'" label="nombre">
                                    </v-select>
                                </div>
                                <div class="form-group col-md-3">
                                    <label>RFC receptor </label>
                                    <input type="text" class="form-control" v-model="Receptor.rfc" placeholder="RFC receptor" readonly>
                                </div>
                                <div class="form-group col-md-3">
                                    <label>C.P. receptor </label>
                                    <input type="text" class="form-control" v-model="Receptor.codigo_postal" placeholder="RFC receptor" readonly>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-10">
                                    <label>Nombre o razon social</label>
                                    <input type="text" class="form-control" v-model="Receptor.nombre" placeholder="Nombre o razon social" readonly>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-10">
                                    <label>Régimen Fiscal</label>
                                    <input type="text" class="form-control disabled" v-model="Receptor.regimen_fiscal" placeholder="Régimen Fiscal" readonly>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control" v-model="factura.uso_factura">
                                        <option :key="index" v-for="(item,index) in UsoFactura" :value="item.id">{{item.c_uso}} {{item.descripcion}}</option>
                                    </select>
                                </div>
                                <div class="form-group col-md-3">
                                    <label>Exportación </label>
                                    <select class="form-control" v-model="factura.exportacion" v-validate="'required'">
                                ...
                                <div class="form-group">
                                    <label>Correo </label>
                                    <input type="text" class="form-control" v-model="Receptor.contacto" placeholder="Correo" readonly>
                                </div>
                                -->
                            </div>
                            <hr>
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select class="form-control" v-model="factura.moneda_id" @change="tipomoneda">
                                        <option :key="index" v-for="(item,index) in Monedas" :value="item.id">{{item.c_moneda}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-4">
                                    ...
                                    <select class="form-control" v-model="factura.formapago">
                                        <option :key="index" v-for="(item,index) in FormaPago" :value="item.id">{{item.clave}} {{item.descripcion}}</option>
                                    </select>
                                </div>
                                <div class="form-group col-md-4">
                                    <label>Metodo de pago</label>
                                    <select class="form-control" v-model="factura.metodopago">
                                        <option :key="index" v-for="(item,index) in MetodoPago" :value="item.id">{{item.c_metodopago}} {{item.descripcion}}</option>
                                    </select>
                                </div>
                                <div class="form-group col-md-4">
                            ...
                            <div class="form-row" v-show="factura.tipo_factura_id == 4">
                                <div class="form-group col-md-4">
                                    ...
                                    <input type="text" v-validate="'integer'" class="form-control" name="numero de operacion" data-vv-name="numero de operacion" v-model="factura.num_operacion">
                                    <span class="text-danger">{{errors.first('numero de operacion')}}</span>
                                </div>
                                <div class="form-group col-md-4">
                                    <label>Banco ordenante</label>
                                    <input type="text" class="form-control" name="banco ordenante" data-vv-name="banco ordenante" v-model="factura.ban_ordenante">
                                </div>
                                <div class="form-group col-md-4">
                                    <label>Cuenta ordenante</label>
                                    <input type="text" class="form-control" name="cuenta ordenante" data-vv-name="cuenta ordenante" v-model="factura.cuenta_ordenante">
                                </div>
                            </div>
                            <div class="form-row" v-show="factura.tipo_factura_id == 4">
                                <div class="form-group col-md-4">
                                    <label>RFC banco ordenante</label>
                                    <input type="text" class="form-control" name="RFC banco ordenante" data-vv-name="RFC banco ordenante" v-model="factura.rfc_cuenta_ordenante">
                                </div>
                                <div class="form-group col-md-4">
                                    <label>RFC banco beneficiario</label>
                                    <input type="text" class="form-control" name="RFC banco beneficiario" data-vv-name="RFC banco beneficiario" v-model="factura.rfc_cuenta_beneficiario">
                                </div>
                            </div>
                            <div class="form-row" v-show="factura.tipo_factura_id == 4">
                                <div class="form-group col-md-4">
...
export default
{
    ...
    methods:
    {
        ...
        getData()
        {
            ...
            {
                this.list_emisores = response.data.data;
            });
        },
        ...
        llenarDatosCliente()
        {
            ...
            axios.get('/clientes/' + this.cliente.id).then(res =>
            {
                ...
                if (res.data.status)
                {
                    ...
                    {
                        if (res.data.cliente.regimen_fiscal.length != 3)
                            toastr.warning("Sin Régimen Fiscal");
                        this.Receptor.nombre = res.data.cliente.nombre;
                        this.Receptor.rfc = res.data.cliente.rfc;
                        this.Receptor.codigo_postal = res.data.cliente.codigo_postal;
                        this.Receptor.contacto = res.data.cliente.contacto;
                        this.Receptor.regimen_fiscal = res.data.cliente.regimen_fiscal + " - " +
                            res.data.cliente.regimen_fiscal_nombre;
                    }
                }
                else
                {
                ...
                if (result)
                {
                    ...
                    axios(
                    {
                        ...
                        data:
                        {
                            ...
                            exportacion: this.factura.exportacion,
            ...
            }).then(result =>
            {
                if (result.value)
                {
                    ...
                    axios.get('/partidafactura/' + data.id + '?query=%7B%7D&limit=10&ascending=1&page=1&byColumn=1').then(response =>
                    {
                        ...
                        {
                            axios.get('/sellartimbrarfactura/' + data.id).then(res =>
                            {
                                if (res.data.status)
                                {
                                    this.isLoading = false;
                                    ...
                                    this.verfacturas(this.emisor_id);
                                }
                                else
                                {
                                    this.isLoading = false;
        ...
        async TimbrarV4(data, prueba = true)
        {
            let modo = prueba ? "Modo de Prueba" : "";
            ...
            if (continuar.value == null) return; // Fuera
            if (!continuar.value) return; // Cancelar
            let res = await axios.get('/partidafactura/' + data.id + '?query=%7B%7D&limit=10&ascending=1&page=1&byColumn=1');
            // Comprobar que tenga conceptos
            if (res.data.count == 0)
            {
        ...
        descargarxml(data)
        {
            axios.get("/descargarfacturareportexml/" + data.archivo).then(res =>
            {
                if (!res.data.status)
                {
         ...
         */
        async CancelarFactura(data, prueba = true)
        {
            let result = await Swal.mixin(
                {
                    input: "text",
            ...
            let id = data.id; // Id de factura
            let motivo = result.value[0];
            let uuid_reemplazo = result.value[1];
