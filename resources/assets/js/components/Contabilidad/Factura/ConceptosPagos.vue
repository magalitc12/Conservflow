          ...
          <template slot="id" slot-scope="props">
            <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
              <div class="btn-group dropup" role="group">
                ...
                <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                  <button  type="button" :disabled="datos.timbrado == 1" @click="abrirModal('partidas','actualizar',props.row)" class="dropdown-item">
                    <i class="icon-pencil"></i>&nbsp; Ver Concepto
                  </button>
                  <button  type="button" :disabled="datos.timbrado == 1" @click="eliminar(props.row)" class="dropdown-item">
                    <i class="fas fa-list-ol"></i>&nbsp; Eliminar
                  </button>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-inline">
              <input type="checkbox" v-model="pago_dolares">&nbsp; Pago en USD 
            </div>
            <br>

            <div class="form-row" v-show="pago_dolares">
              <div class="form-group col-md-2">
                <label>Total USD</label>
                <input @input="CalcularTotalDLS($event)" type="text" class="form-control" v-model="partidas_pagos.total_usd">
              </div>
              <div class="form-group col-md-2">
                <label>Tipo Cambio</label>
                <input @input="CalcularTotalTC($event)" type="text" class="form-control" v-model="partidas_pagos.tipo_cambio_dr" placeholder="0.05">
              </div>
              <div class="form-group col-md-2">
                <label>Total</label>
                <input disabled type="text" class="form-control" v-model="partidas_pagos.importe_pagado_mx" placeholder="0">
              </div>
              <div class="form-group col-md-4">
                  <label>Metodo de pago</label>
                  <select class="form-control" v-model="partidas_pagos.metodo_pago_dr">
                      <option :key="index" v-for="(item,index) in list_metodos_pago" :value="item.id">
            ...
            <br>
            <div class="form row">
                <label class="col-md-4">Objeto Imp.</label>
                <select class="form-control col-md-3" v-model="partidas_pagos.obj_imp">
            ...
            <div class="form-row">
              ...
              <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control" name="saldo insoluto" data-vv-name="saldo insoluto"
                 v-validate="'decimal:2'" v-model="partidas_pagos.saldo_insoluto">
                <span class="text-danger">{{errors.first('saldo insoluto')}}</span>
              </div>
              <br>
                <span v-show="pago_dolares" class="text-danger">
          ...
          <div class="modal-footer">
            <vue-element-loading :active="isLoading"/>
            <button type="button" class="btn btn-outline-dark" @click="cerrarModalUno()">
              <i class="fas fa-times"></i>&nbsp;Cerrar
            </button>
            <button type="button" v-if="tipoAccionUno==1" class="btn btn-secondary" @click="validaUno(); guardarUno(1);">
              <i class="fas fa-save"></i>&nbsp;Guardar
            </button>
            <button type="button" v-if="tipoAccionUno==2" class="btn btn-secondary" @click="validaUno();guardarUno(0)">
              <i class="fas fa-save"></i>&nbsp;Guardar  <!-- Actusliar -->
            </button>
          </div>
        </div>
        <!-- /.modal-content -->
  ...
  methods : {
    getData(data){
      ...
      */
      ...
      CalcularTotalDLS(e)
      {
        let dls=e.target.value;
        let tc=this.partidas_pagos.tipo_cambio_dr;
        let total=(dls/tc).toFixed(2);
      ...
      CalcularTotalTC(e)
      {
        ...
        let dls=this.partidas_pagos.total_usd;
        let total=(dls/tc).toFixed(2);
        if(isNaN(total)) total=0;
        this.partidas_pagos.importe_pagado_mx=total;
      },
      ...
      ObtenerMetodosPago()
      {
          ...
          {
              this.list_metodos_pago = response.data.data;
          });
      },
      ...
      guardarUno(nuevo){
        ...
        this.$validator.validate().then(result => {
          ...
          if (result) {
            ...
            if(this.pago_dolares)
            {
              // Si se marcó 'pago en dolares' se guardan los datos correspondientes DR
              data.append("total_usd",this.partidas_pagos.total_usd);
              data.append("tipo_cambio_dr",this.partidas_pagos.tipo_cambio_dr);
              data.append("metodo_pago_dr",this.partidas_pagos.metodo_pago_dr);
              data.append("importe_pagado_mx",this.partidas_pagos.importe_pagado_mx);
              data.append("pago_dolares" , this.pago_dolares?1:0);
            }

            data.append("factura_id" , this.partidas_pagos.factura_id === '' ? '' :this.partidas_pagos.factura_id.id);
            data.append("num_parcialidad" , this.partidas_pagos.num_parcialidad);
            data.append("saldo_anterior" , this.partidas_pagos.saldo_anterior);
            data.append("importe_pagado" , this.partidas_pagos.importe_pagado);
            data.append("saldo_insoluto" , this.partidas_pagos.saldo_insoluto);
            data.append("uuid" , this.partidas_pagos.uuid);
            data.append("serie" , this.partidas_pagos.serie);
            data.append("folio" , this.partidas_pagos.folio);
            data.append("obj_imp", this.partidas_pagos.obj_imp);
            data.append("partidas_facturas_id" , this.tableData.data[0].id);
            axios.post('/partidafacturapagos',data).then(response=> {
              this.partidas_pagos.total_usd=0;
              // this.partidas_pagos.tipo_cambio_dr=0;
              this.partidas_pagos.metodo_pago_dr=0;
              me.isLoading = false;
              me.cerrarModalUno();
              toastr.success(nuevo ? 'Concepto agregada correctamente' : 'Concepto actualizada correctamente','Correcto');
              me.getData(response.data);
