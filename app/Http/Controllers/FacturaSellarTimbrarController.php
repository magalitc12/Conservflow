...
class FacturaSellarTimbrarController extends Controller
{
  ...
  public function sellartimbrar($id)
  {
    ...
    $partidas_pagos = DB::table("partidas_facturas_pagos as pfp")
      ->where('pfp.partidas_facturas_id', $partidas[0]->id)
    ...
    // dd($ini_file_str);
    Storage::disk('local')->put('Facturas/' . $id . '-' . $date . ".ini", $ini_file_str);
  ...
  public function sellartimbrarprueba($id)
  {
    ...
    ->leftJoin("sat_cat_metodopago as scm","scm.id","metodo_pago_dr")
      ->select(
        'partidas_facturas_pagos.*',
        "scm.c_metodopago")
      ->get();
    ...
    $ini_file_str=$this->Principal($factura, $partidas, $partidas_pagos);
    // cambiar el disk por ftp cuando no sea local
    Storage::disk('local')->put('Facturas/' . $id . '-' . $date . "p.ini",$ini_file_str );
    if ($factura->emisor_id == '1')
    {
      $output = shell_exec("cd /home/libfaclin/;./sellartimbrar.sh $id-$date 2> ehoyc.txt;cat ehoyc.txt;");
      // $output = shell_exec("cd /home/libfaclin/;./sellartimbrar.sh $id-$date 2> ehoyc.txt;cat ehoyc.txt;");
    ...
    if ($output == null)
    {
      ...
      $facturas->archivo = $id . '-' . $date;
      $facturas->total = $factura->tipo_factura_id == 4 ?  $total_pagos_f : (($suma_subtotal - $suma_descuento) + $suma_impuesto);
      Utilidades::auditar($facturas, $facturas->id);
      $facturas->save();
   ...
   * @param  Object  $factura        [Array recibido en la función]
  ...
  public function Principal($factura, $partida, $partidas_pagos)
  {
    ...
    $variable  = ';ini' . PHP_EOL . ';encoding=utf8' . PHP_EOL . PHP_EOL .
      ...
      //($factura->sctf_c_tipofactura == 'E' ? $this->Relacionados($factura) : $factura->tipo_relacion != '' ? $this->Relacionados($factura) : $factura->tipo_relacion != '' ? $this->Relacionados($factura) : '') 
      $val. PHP_EOL .
   ...
   * @param  Object  $factura      [Array recibido en la funcion]
  ...
  public function CReceptor($id, $uso_cfdi)
  {
    ...
    $receptor_ = Clientes::where('id', $id)->first();
    ...
    try
    {
      ...
      $factura = \App\Factura::where('factura.id', $valores[0])
        ...
        ->select(
          ...
          'c.rfc AS rfc_c',
          'dg.rfc AS rfc_e',
          DB::raw("CONCAT(scuc.c_uso,' ',scuc.descripcion) AS uso_cfdi"),
  ...
  public function descargarprefactura($id)
  {
    try
    {
      ...
      $suma_impuesto=number_format($suma_impuesto,2,".","");

      $total = number_format(((($suma_subtotal - $suma_descuento) + $suma_impuesto) - $suma_retencion),2);
      
      $cambio = Utilidades::valorEnLetrasFactura(str_replace(",","",$total), $factura->moneda_id);
      $v_l_total_pagos = Utilidades::valorEnLetrasFactura(round($suma_partidas_pagos, 2), $factura->moneda_id);
  ...
  {
    // code...
    $receptor_ = Clientes::where('id', $factura->cliente_id)->first();
    ...
    {
      $aux_tipo_cambio="";
      if($value->tipo_cambio_dr!=0) // USD a MX
      {
        $monto=round($value->importe_pagado/$value->tipo_cambio_dr,2);
        $moneda="USD";
        $tipo_cambio=$value->tipo_cambio_dr;
        $aux_tipo_cambio='TipoCambioDR =' . $tipo_cambio;
        $metodo_pago=$value->c_metodopago;
      }else
      {
        $metodo_pago=$factura->scmp_c_metodopago;
        $tipo_cambio=$factura->scm_c_moneda;
        $moneda=$factura->scm_c_moneda;
        $monto=$value->importe_pagado;
      }
      $complemento .= '[cfdi:Comprobante/cfdi:Complemento/pago10:Pagos/pago10:Pago]' . PHP_EOL .
        ...
        'MonedaDR = ' .$moneda. PHP_EOL .  // Cambiar si es otra moneda
        // 'MonedaDR =' . $factura->scm_c_moneda . PHP_EOL .
        $aux_tipo_cambio. PHP_EOL .
        // 'TipoCambioDR =' . $factura->scm_c_moneda . PHP_EOL .
        'MetodoDePagoDR =' . $metodo_pago . PHP_EOL .
        // 'MetodoDePagoDR =' . $factura->scmp_c_metodopago . PHP_EOL .
        'NumParcialidad = '.$value->num_parcialidad . PHP_EOL .
