...
class SalidaController extends Controller
{
  ...
  public function sitios($id)
  {
    ...
    $salidas = DB::table('salidassitio')
      ...
      ->select(
        'salidassitio.*',
        'proyectos.nombre AS nombrep',
        'proyectos.nombre_corto AS nombrec',
        'tipo_salidas.nombre AS salidan',
  ...
  public function store(Request $request)
  {
    try
    {
      ...
      if ($request->tiposalida_id == 1) //Taller
      {
        ...
        Utilidades::auditar($salidas, $salidas->id);
        $salidas->save();
      }
      if ($request->tiposalida_id == 2) //sitio
      {
        ...
        Utilidades::auditar($salidasitio, $salidasitio->id);
        $salidasitio->save();
      }
      if ($request->tiposalida_id == 3) //Resguardo
      {
        ...
        Utilidades::auditar($salidaresguardo, $salidaresguardo->id);
...
ob_start();
    ...
    return Excel::download(new SalidaExport($id), 'Reporte_Salidas.xlsx');
  }
