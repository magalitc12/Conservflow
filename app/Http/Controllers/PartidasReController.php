...
class PartidasReController extends Controller
{
  ...
  public function show($id)
  {
    $data_art = DB::table('partidas_requisiciones')
      ...
      ->select(
        ...
        'a.centro_costo_id AS centro_costo',
        'a.unidad',
        'a.id AS ida',
        'r.id AS rid',
        DB::raw("concat_ws(' ',a.nombre,a.descripcion) as descripciona"),
    ...
    $data_serv = DB::table('partidas_requisiciones')
      ...
      ->select(
        'partidas_requisiciones.*',
        's.centro_costos_id AS centro_costo',
        's.id AS ida',
        'r.id AS rid',
  ...
  public function getApartados($id)
  {
    $data_art = DB::table('partidas_requisiciones')
      ...
      ->leftJoin('proyectos AS p', 'p.id', '=', 'r.proyecto_id')
      ->select(
        'partidas_requisiciones.cantidad_almacen as cantidad_compra',
        'partidas_requisiciones.requisicione_id',
        'partidas_requisiciones.articulo_id',
        'partidas_requisiciones.servicio_id',
        'partidas_requisiciones.pda',
        'a.centro_costo_id AS centro_costo',
        'a.unidad',
        'a.id AS ida',
        'r.id AS rid',
  ...
  public function store(Request $request)
  {
    try
    {
      ...
      if ($request->pendiente != 0)
      {
        ...
        Utilidades::auditar($par_entrada, $par_entrada->id);
        $par_entrada->save();
        ...
        else
        {
          ...
          if ($request->documentacionreid != '')
          {
            for ($i = 0; $i < count($request->documentacionreid); $i++)
            {
              ...
              $documentos->save();
              Auditoria::AuditarCambios($documentos);
            }
          }
  ...
  public function edit($id)
  {
    ...
    if ($articulo != "null")
    {
      ...
      if (count($reqhasoc) != 0)
      {
        ...
        Utilidades::auditar($actualizar, $actualizar->id);
        $actualizar->save();
        ...
        $actualizar2->save();
        Utilidades::auditar($actualizar2, $actualizar2->id);
      }
      /************************************************************************/
    ...
    if ($request->partidarequisicione_art != null)
    {
      ...
      if ($request->documento_id != '')
      {
        for ($i = 0; $i < count($request->documento_id); $i++)
        {
          ...
          Utilidades::auditar($documentos, $documentos->id);
          $documentos->save();
        }
      }
