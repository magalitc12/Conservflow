...
class LotesBarrasSalidasController extends Controller
{
  ...
  public function GenerarCodigos($id)
  {
    try
    {
      ...
      for ($i = 0; $i < count($barras) - 1; $i++)
      {
        ...
        $n_nombre = strlen($rhoc->descripcion);

        $descripcion = substr($rhoc->descripcion, 0, 80) . ($n_nombre > 80 ? "..." : "");
        $array_codigos[] = [
          "img" => $this->Codigo($codigo),
          "descripcion" => $descripcion,
          "codigo" => $codigo,
        ];
      }
      $codigos = array_chunk($array_codigos, 2);
      // dd($codigos);
      $pdf = PDF::loadView('pdf.codigoentrada', compact('codigos'));
      $pdf->setPaper("letter", "portrait");
      return $pdf->stream("Códigos - Almacén");
    }
    catch (Exception $e)
    {
      Utilidades::errors($e);
  ...
  {
    return DNS1D::getBarcodePNG($nombre, 'C39', 3, 90, array(0, 0, 0));
  }
