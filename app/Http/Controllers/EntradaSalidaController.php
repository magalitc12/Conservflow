...
use \App\LoteAlmacen;
use \App\Existencia;
use App\Exports\Almacen\ReporteRetornosExport;
use \App\Movimiento;
...
class EntradaSalidaController extends Controller
{
    ...
    public function GuardarRetorno(Request $request)
    {
        ...
        try
        {
            ...
            // existencias
            $existencias = Existencia::find($request->loteId);
            if ($existencias)
            {
            ...
            //Stock articulos
            $stock = StockArticulo::where("stocke_id", "=", $loteAlmacen->stocke_id)
    ...
    /**
     * Descargar reporte de retornos de almacen
     */
    public function ReporteRetornos()
    {
        try
        {
            ob_end_clean();
            ob_start();
            return Excel::download(new ReporteRetornosExport(), "Retornos_General.xlsx");
        }
        catch (Exception $e)
        {
            Utilidades::errors($e);
