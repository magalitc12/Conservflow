    ...
    try
    {
      ...
      $partidasRE = \App\PartidaRe::where('partidas_requisiciones.requisicione_id', '=', $id)
        ...
        ->select(
          ...
          DB::raw("concat_ws(' ',articulos.nombre,articulos.descripcion) as descripcionarticulo"),
    ...
    try
    {
      ...
      foreach ($partidasRE as $key => $value)
      {
        ...
        if (!is_null($articulo_id))
        {
          $doc = DB::table('partidarequisicion_documentos')
            ->leftJoin('documentos_proveedores AS DP', 'DP.id', '=', 'partidarequisicion_documentos.documento_id')
            ->select('partidarequisicion_documentos.documento_id', 'DP.nombre_corto', "DP.nombre")
            ->where('partidarequisicion_documentos.partidarequisicione_req', '=', $id)
            ->where('partidarequisicion_documentos.partidarequisicione_art', '=', $articulo_id)
            ->orderBy("DP.nombre_corto")
            ->get();
        }
        if (!is_null($servicio_id))
        {
      ...
      // Obtener documentos
      $documentos = DB::table("partidarequisicion_documentos as pd")
        ->select("dp.nombre_corto", "dp.nombre")
        ->join("documentos_proveedores as dp", "dp.id", "pd.documento_id")
        ->where("pd.partidarequisicione_req", $id) // Requi
        ->orderBy("dp.nombre_corto")->distinct()->get()->toArray();
      $documentos = array_chunk($documentos, 3);
