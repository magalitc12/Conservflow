...
class PartidasFacturaController extends Controller
{
  ...
  public function store(Request $request)
  {
    try
    {
      ...
      Utilidades::auditar($partida_factura, $partida_factura->id);
  ...
  {
    $partida_factura = \App\PartidasFactura::leftJoin('sat_cat_prodser AS SCPS', 'SCPS.id', '=', 'partidas_factura.productos_servicios_id')
      ->join('sat_cat_unidades AS SCU', 'SCU.id', '=', 'partidas_factura.unidad_id')
      ->join('factura AS F', 'F.id', '=', 'partidas_factura.factura_id')
      ->select(
        'partidas_factura.*',
        'SCPS.clave AS scps_clave',
        'SCPS.descripcion AS scps_descripcion',
        'SCU.c_unidad AS scu_unidad',
        'SCU.nombre AS scu_nombre'
      )
  ...
  public function CambiarObservaciones(Request $request)
  {
    try
    {
      $partida = \App\PartidasFactura::find($request->id);
    ...
    {
      return Status::Error($e, "guardar las observaciones");
    }
  }
}
