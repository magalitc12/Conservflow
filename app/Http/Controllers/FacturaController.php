...
class FacturaController extends Controller
{
  ...
  {
    $facturas = \App\Factura::join('datosgenerales AS dg', 'dg.id', '=', 'factura.emisor_id')
      ...
      ->select(
        'factura.*',
        'dg.rfc AS rfc_emisor',
        'dg.razon_social AS razons_emisor',
        'dg.nombre AS nombre_emisor',
        'dg.regimenfiscal AS regimenfiscal_emisor',
        'sctf.c_tipofactura',
        'sctf.descripcion AS descripcion_tipofactura',
        'c.nombre AS nombre_razons_receptor',
        'c.rfc AS rfc_receptor',
        'c.contacto AS correo_receptor',
        'scuc.c_uso',
        'scuc.descripcion AS usocfdi_descripcion',
        'scm.c_moneda',
        'scm.descripcion AS monedas_descripcion',
        'scfp.clave AS clave_formapago',
        'scfp.descripcion AS descripcion_formapago',
        'scmp.c_metodopago',
        'scmp.descripcion AS descripcion_metodopago',
        'f.serie AS serie_relacionado',
        'f.folio AS folio_relacionado',
  ...
  {
    $factura = \App\Factura::join('datosgenerales AS dg', 'dg.id', '=', 'factura.emisor_id')
      ->join('sat_cat_tipofactura AS sctf', 'sctf.id', '=', 'factura.tipo_factura_id')
      ->join('clientes AS c', 'c.id', '=', 'factura.cliente_id')
      ->join('sat_cat_usocfdi AS scuc', 'scuc.id', '=', 'factura.uso_factura')
      ->join('sat_cat_monedas AS scm', 'scm.id', '=', 'factura.moneda_id')
  ...
  public function store(Request $request)
  {
    try
    {
      ...
      $factura->empleado_registra_id = Auth::user()->id;
      Utilidades::auditar($factura, $factura->id);
      $factura->save();
      Auditoria::AuditarCambios($factura);
  ...
  {
    $clientex = Clientes::where('rfc', 'XEXX010101000')->first();
  ...
  public function verfacturauno($id)
  {
    ...
    $permitidos = [50, 150];
    if (!in_array(Auth::user()->empleado_id, $permitidos))
    {
      $facturas->where('factura.empleado_registra_id', Auth::user()->id);
    }
   ...
   */
  public function descargarfacturaxml($factura_id)
  {
    try
    {
      // Buscar factura
      $del = substr($factura_id, 3);
      $id = str_replace($del, "", $factura_id);
      $fac = Factura::find($id);
      if ($fac == null) return Status::Error2("No encontrada");

      $nombre = $fac->serie . "-" . $fac->folio;
      $file = "Facturas/" . $factura_id . "tim.xml"; // ruta del app al xml
      $path_to_file = storage_path("app/") . $file; // Completa

      if (!file_exists($path_to_file)) return Status::Error2("No encontrada");
      $file = Storage::disk('local')->get($file);
      $data = (object)["xml" => $file, "nombre" => $nombre];
      return Status::Success("factura", $data);
    }
    catch (Exception $e)
    {
      Utilidades::errors($e);
