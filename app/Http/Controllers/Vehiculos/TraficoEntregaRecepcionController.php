...
use App\VehiculosModels\TraficoEntregaRecepcion;
use App\VehiculosModels\TraficoEntregaRecepcionImg;
...
class TraficoEntregaRecepcionController extends Controller
{
  ...
  public function Guardar(Request $request)
  {
    try
    {
      ...
      DB::beginTransaction();
      $data = new TraficoEntregaRecepcion();
      ...
      {
        $imagenes = new TraficoEntregaRecepcionImg();
  ...
  public function GuardarRep(Request $request)
  {
    try
    {
      ...
      $data_rel = TraficoEntregaRecepcion::where('id', $request->id_rel)->first();
  ...
  public function Actualizar(Request $request)
  {
    try
    {
      // return response()->json(gettype(json_encode($request->accesorios)));
      DB::beginTransaction();
      $data = TraficoEntregaRecepcion::where('id', $request->id)->first();
      $data->accesorios = json_encode($request->accesorios);
      $data->accesorios_e = json_encode($request->accesorios_e);
      $data->accesorios_i = json_encode($request->accesorios_i);
      $data->carroceria = json_encode($request->carroceria);
  ...
  public function get()
  {
    $data = TraficoEntregaRecepcion::join('empleados AS ee', 'ee.id', 'entrega_recepcion_vehiculos.entrega')
  ...
  public function deleteImg($id)
  {
    try
    {
      $img = TraficoEntregaRecepcionImg::where('id', $id)->first();
      $img->condicion = 0;
  ...
  public function Descargar($id)
  {
    try
    {
      ...
      $data_r = TraficoEntregaRecepcion::join('empleados AS ee', 'ee.id', 'entrega_recepcion_vehiculos.entrega')
        ->join('empleados AS er', 'er.id', 'entrega_recepcion_vehiculos.recibe')
        ->join('empleados AS eu', 'eu.id', 'entrega_recepcion_vehiculos.usuario')
        ->select(
          'entrega_recepcion_vehiculos.*',
      ...
      $conductor = DB::table('choferes AS c')->join('empleados AS e', 'e.id', 'c.empleado_id')
        ->select('c.*', DB::raw("CONCAT(e.nombre,' ',e.ap_paterno,' ',ap_materno) AS nombre_empleado"))
        ->where('c.empleado_id', $data->usuario)
      ...
      $arreglo = [
        ...
        'lugar_entrega' => mb_strtoupper($datos->lugar_entrega),
        'proyecto' => $datos->proyecto->name,
        'destinatario' => mb_strtoupper($datos->destinatario),
        //////
        ...
        'lugar_recepcion' => isset($data_r) == false ? '' : mb_strtoupper($datos_r->lugar_entrega),
      ];

      $pdf = PDF::loadView('pdf.entregarecepcionv', compact(
        'arreglo',
        'data',
        'data_r',
        'accesorios_e',
