...
use App\VehiculosModels\SolicitudVehiculo;
...
class SolicitudVehiculoController extends Controller
{
    ...
    public function Guardar(Request $request)
    {
        try
        {
            ...
            $datos = LimpiarInput::LimpiarCampos(
                $request->all(),
                ["folio", "tiempo", "ciudad", "estado", "poblacion", "necesidad"]
            );
            $soli = null;
            // Guardar soli
            if ($request->id == null)
            {
                $soli = new SolicitudVehiculo($datos);
    ...
    public function GenerarReporte($id)
    {
        try
        {
            ...
            // Datos generales del reporte(diseño)
            if ($solicitud->empresa == 1) //conser
            {
                $pdf = "pdf.solicitudvehiculoconser";
            }
            else //csct
            {
                $pdf = "pdf.solicitudvehiculo";
            }
            $pdf = PDF::loadView($pdf, compact("solicitud", "responsables"));
