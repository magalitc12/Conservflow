...
use App\VehiculosModels\VehiculosMttoAnual;
use Exception;
...
class VehiculosMttoAnualControlller extends Controller
{
    ...
    public function Guardar(Request $request)
    {
        try
        {
            $datos=LimpiarInput::LimpiarCampos($request->all(),["servicio","observaciones"]);
            if ($request->id == null)
            {
                $mtto = new VehiculosMttoAnual($datos);
    ...
    public function Descargar($data)
    {
        try
        {
            ...
            $pdf = $empresa == 1 ? "pdf.vehiculomttoanualconser" : "pdf.vehiculomttoanual";
            $pdf = PDF::loadView($pdf, compact("empresa", "mttos", "anio"));
