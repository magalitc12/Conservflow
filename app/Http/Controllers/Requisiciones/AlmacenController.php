...
class AlmacenController extends Controller
{
        ...
        try
        {
            ...
            switch ($requi->tipo->ruta)
            {
                case Tipo::$MATERIALES:
                    $partidas = PartidaMaterial::with("unidadMedida")->byRequisicion($requi->id)
                        ->soloMateriales()->get();
                    break;
                default:
    ...
    public function rechazar(Request $request)
    {
        try
        {
            ...
            $requisicion->condicion = Requisicion::$RECHAZADO_ALMACEN;
            ...
            switch ($requisicion->tipo->ruta)
            {
                case Tipo::$MATERIALES:
                    $partidas_pendientes = PartidaMaterial::byRequisicion($requisicion->id)
                        ->soloMateriales()->pendienteAlmacen()->count();
                    break;
                default:
