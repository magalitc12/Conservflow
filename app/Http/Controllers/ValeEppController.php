...
class ValeEppController extends Controller
{
    ...
    public function getDetalle($id)
    {
        try
        {
            ...
            $epp_partidas = \App\PartidasValeEpp::join('articulos AS a', 'a.id', 'partidas_vale_epp.articulo_id')
    ...
    public function guardar(Request $request)
    {
        try
        {
            ...
            foreach ($request->datos as $key => $value)
            {
                ...
                $epp_partidas->motivo = $value['motivo'];
                $epp_partidas->observaciones = $value['observaciones'];
    ...
    {
        $dia = date('Y-m-d');
        $epp_partidas = \App\PartidasValeEpp::join('empleados_vale_epp AS evp', 'evp.id', 'partidas_vale_epp.empleado_vale_id')
            ->join('articulos AS a', 'a.id', 'partidas_vale_epp.articulo_id')
            ->join('empleados AS e', 'e.id', 'partidas_vale_epp.autoriza_id')
    ...
    /**
     * Genera el formato de la entrega de EPP, de acuerdo al tipo seleccionado
     */
    public function descargar($data)
    {
        $dts = explode("&", $data);
        if (count($dts) < 2) return  "<h1>Datos incompletos <h1>";
        $id = $dts[0]; // Id de empleado
        $tipo_formato = $dts[1]; // Tipo de formato
        $emp = \App\EmpleadoValeEpp::where('empleado_id', $id)->select('id')->get();
        $empleado_data = DB::table('empleados AS e')
            ->leftJoin('puestos AS p', 'p.id', 'e.puesto_id')
            ->select(
        ...
        $epp_partidas = \App\PartidasValeEpp::join('articulos AS a', 'a.id', 'partidas_vale_epp.articulo_id')
            ->join('empleados AS e', 'e.id', 'partidas_vale_epp.autoriza_id')
            ->select('partidas_vale_epp.*', 'a.descripcion', DB::raw("CONCAT(e.nombre,' ',e.ap_paterno,' ',e.ap_materno) AS autoriza"))
            ->whereIn('empleado_vale_id', $a);
        if ($tipo_formato == 2)
        {
            // Desde fecha de formato
            $epp_partidas->where("fecha", ">=", "2021-06-21");
            $revision="01";
            $fecha_emision = "21.JUN.21";
            $firma="Aceptación";
        }
        else
        {
            $fecha_emision = "01.ABR.20";
            $revision="00";
            $firma="Firma Recibido";
            $epp_partidas->where("fecha", "<", "2021-06-21");
        }
        $epp_partidas = $epp_partidas->get();

        $pdf = PDF::loadView('pdf.valeepp',
        compact('empleado_data', 'epp_partidas', "tipo_formato", "fecha_emision","revision","firma"));
    ...
    public function Autorizar(Request $request)
    {
        try
        {
            ...
            // Obtener empleado 
            $entrega = DB::table("partidas_vale_epp as pve")
                ->join("empleados_vale_epp as eve", "eve.id", "pve.empleado_vale_id")
                ->select(
                    "pve.id",
                    "eve.empleado_id"
                )
                ->where("pve.id", $request->resguardo_id)
                ->first();
            if ($entrega->empleado_id != $request->empleado_id)
            {
                return response()->json(["status" => false, "mensaje" => "Persona no autorizada"]);
            }
            $entrega = PartidasValeEpp::find($request->resguardo_id);
            $entrega->autorizado = true;
            $entrega->update();
        ...
        {
            dd($e);
            return Status::Error($e, "autorizar la entrega");
        }
    }
