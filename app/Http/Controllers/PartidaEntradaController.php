...
class PartidaEntradaController extends Controller
{
   ...
   * @param  string $queries [description]
  ...
  public function store(Request $request)
  {
    try
    {
      ...
      if (isset($partida_entrada_busqueda) == true)
      {
        ...
        // Obtener cantidad total de todas las entradas de la rho
        $part_entr = DB::table('partidas_entradas')->select(
          DB::raw('SUM(cantidad) as cantidad')
        )
          ->where('req_com_id', $request->req_com_id)
          ->first();
  ...
  {
    // Buscar la rho de la entrada
    $requisis = \App\Requisicionhasordencompras::where('id', '=', $orden_compra_id)->first();
    // Actualizar cantidad pendiente por entrar
    // Cantidad pedida -(existente +entrada)
  ...
  public function condicion($orden_compra_id, $cantidad)
  {
    ...
    // CAntidad_entrada=0? Ya no hay pendientes
  ...
  // FIXME: Eliminar
  /**
   * Eliminar todas las entradas de la Oc ingresada
   */
  public function EliminarEntrada($ids)
  {
    try
    {
      DB::beginTransaction();
      ...
      {
        $asd = $id;
        // Buscar las entradas de la oc
        $entrada = Entrada::join("ordenes_compras as oc", "entradas.orden_compra_id", "oc.id")
          // ->where("proyecto_id", 342)
          ->select("entradas.id")
          ->where("entradas.orden_compra_id", $id)
          ->first();
        $partidas = PartidaEntrada::where("entrada_id", $entrada->id)->get();
        foreach ($partidas as $p)
        {
          $this->eliminarPartidaEntrada($p->id);
        }
  ...
  public function eliminarPartidaEntrada($id)
  {
    try
    {
      ...
      $partida_entrada = PartidaEntrada::find($id);
      // $partida_entrada->delete();
      ...
      if ($lote != null)
      {
        ...
        $movimiento->tipo_movimiento = 'Del Entrada';
  ...
  */
   ...
   * @param  Object  $partidaentrda [description]
   * @return Boolean               [description]
   */
  ...
  /**
   * [ calcula el precio del lote entrante utilizando los datos de la orden de compra]
   * @param  Object $request [description]
   * @param  Object $lote    [description]
   * @return Response          [description]
   */
  ...
  public function actualizarPrecioEntradaInterna(Request $request)
  {
    ...
    Utilidades::auditar($partidas_e, $partidas_e->id);
    $partidas_e->save();
