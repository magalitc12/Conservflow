...
class EntradaPdfController extends Controller
{
	...
	{
		$ids = Auth::id();
		$entrada = DB::table('entradas')->select(
			'entradas.*',
		...
		try
		{
			...
			$partida_entrada = DB::table('partidas_entradas')
				...
				->select(
					...
					'A.unidad AS aunidad',
					'A.id AS aid',
					DB::raw("IF(RHC.comentario is null,A.descripcion,
          concat(A.descripcion,' - ',RHC.comentario)) as descripcion"),
					'PRO.nombre_corto AS proyecto',
					'A.marca AS amarca',
					'OC.id AS idcompra',
					'OC.folio AS foliocompra',
			...
			if (strtotime($entrada->fecha) >= strtotime("2024-01-29"))
			{
				$emision = "29.ENE.24";
				$revision = "01";
				$rfc = "CON1912026U2";
